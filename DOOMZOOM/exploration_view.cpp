...
ExplorationView::ExplorationView(Map& map, Player& player, Camera& camera,
                               Inventory& active,
...
Component ExplorationView::getComponent(ScreenInteractive& screen) {
  buildUI(screen);
  return main_component_;
}

  void ExplorationView::moveEntities() {
...
int ExplorationView::checkEnemyCollision() const {
...
void ExplorationView::buildUI(ScreenInteractive& screen) {
  ...
  auto reserve_column = Renderer(reserve_menu, [=] {
    return vbox({
        ...
        separator(),
        reserve_menu->Render() | size(HEIGHT, LESS_THAN, kInReserveVBoxHeight) |
            border,
    });
  });
