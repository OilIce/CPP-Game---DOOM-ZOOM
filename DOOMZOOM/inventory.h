...
public:
    Inventory(std::vector<std::shared_ptr<Animal>> animals) : team_(std::move(animals)) {
    ...
    std::string getDescription() { return team_.empty() ? "" : team_[selected_].get()->shortInfo(); }

    bool transferTo(Inventory& other, int min_limit = 0, int max_limit = INT32_MAX) {
        if (team_.size() <= min_limit || other.team_.size() >= max_limit) return false;
        ...
        if (selected_ >= team_.size()) selected_ = std::max(0, static_cast<int>(team_.size()) - 1);
