...
void BattleView::buildTurnOrder() {
  all_fighters_.clear();
...
void BattleView::executeAction(int action, int target_idx) {
  int fighter_idx = turn_order_[current_turn_idx_];
  auto& fighter = all_fighters_[fighter_idx];
  Team* my_team = (is_player_team_[fighter_idx]) ? &player_team_ : &enemy_team_;
  Team* opp_team =
...
void BattleView::enemyTurn() {
  ...
  std::uniform_int_distribution<int> act_dist(Action::Attack, Action::Ability2);
